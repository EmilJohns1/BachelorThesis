...
class Clusterer:
    ...
    def update(self, X: np.ndarray, X_rewards: np.ndarray):
        ...
        if not self.is_initialized:
            ...
            self.mu = np.random.uniform(
                low=min_vals, high=max_vals, size=(self.K, self.D)
            )
        ...
        for _ in range(n_iter):
            ...
            for i in range(self.K):
                ...
                F_i = np.exp(
                    -np.sum((X - self.mu[i]) ** 2, axis=1) / self.sigma
                )  # Shape: (num_states,)
                diff_i = np.sum(
                    (F_i * reward_weights)[:, None] * (X - self.mu[i]), axis=0
                )  # Shape: (D,)
                ...
                weights = np.exp(
                    -np.sum((neighbors - self.mu[i]) ** 2, axis=1) / self.sigma
                )  # (K-1,)
                # Sum influence over neighbors
                influence = (
                    2.0
                    * self.lambda_
                    * np.sum(weights[:, None] * (neighbors - self.mu[i]), axis=0)
                )
        ...
        alpha = 3
        F = np.power(
            np.exp(-np.square(X[:, np.newaxis, :] - self.mu).sum(axis=2) / self.sigma),
            alpha,
        )  # Shape: (num_states, K)
        ...
        self.cluster_rewards[mask] *= 1 / (
            1 + weights[mask] / self.cluster_weights[mask]
        )
    ...
    def update_transitions(
        self,
        x: np.ndarray,
        state_action_transitions_from,
        state_action_transitions_to,
        threshold: float,
    ):
        gaussian_weights = np.exp(
            -np.square(x[:, np.newaxis, :] - self.mu).sum(axis=2) / self.sigma
        )  # Shape: (num_states, K)
        ...
        for action in self.actions:
            for from_state, to_state in zip(
                ...
                if (
                    max_weights[from_state] < threshold
                    or max_weights[to_state] < threshold
                ):
                    continue
