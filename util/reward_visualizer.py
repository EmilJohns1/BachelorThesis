...
def plot_multiple_runs(folder_name, title, field, block=True):
    ...
    all_rewards = [rewards[:num_episodes] for rewards in all_rewards]  # Trim to the shortest run
    ...
    plt.fill_between(range(1, len(running_means) + 1), np.array(running_means) - np.array(running_stds), np.array(running_means) + np.array(running_stds), color="black", alpha=0.3, label="Running Mean ± Std")
